package com.dartdl.app.download
...
import com.dartdl.app.download.Task.DownloadState
...
import com.dartdl.app.download.Task.RestartableAction.Download
import com.dartdl.app.download.Task.RestartableAction.FetchInfo
import com.dartdl.app.download.Task.TypeInfo
...
class DownloaderV2Impl(private val appContext: Context) : DownloaderV2, KoinComponent {
    ...
    override fun remove(task: Task): Boolean {
        if (taskStateMap.contains(task)) {
            NotificationUtil.cancelNotification(task.id)
    ...
    private fun Task.fetchInfo() {
        ...
        // Set state to FetchingInfo BEFORE launching the coroutine to avoid race conditions
        val job = scope.launch(Dispatchers.Default) {
            val isInit = App.initializeEngines().await() == true
            if (!isInit) {
                val throwable = IllegalStateException("Initialization failed. Check network.")
            ...
            DownloadUtil.fetchVideoInfoFromUrl(
                    ...
                    .onFailure { throwable ->
                        if (throwable is YoutubeDL.CanceledException || throwable is kotlinx.coroutines.CancellationException || throwable is java.util.concurrent.CancellationException) {
                            NotificationUtil.cancelNotification(notificationId)
                            return@onFailure
                        }
                        task.downloadState = Error(throwable = throwable, action = FetchInfo)
                        val msg = throwable.message?.takeIf { it.isNotBlank() } ?: appContext.getString(R.string.fetch_info_error_msg)
                        NotificationUtil.notifyError(
                                title = viewState.title,
                                text = msg,
                                notificationId = notificationId,
        ...
        val job = scope.launch(Dispatchers.Default) {
            ...
            DownloadUtil.downloadVideo(
                    ...
                    .onFailure { throwable ->
                        ...
                        NotificationUtil.notifyError(
                                title = viewState.title,
                                text = appContext.getString(R.string.fetch_info_error_msg),
                                notificationId = notificationId,
                                report = throwable.stackTraceToString(),
                        )
                    }
    ...
    private fun Task.cancelImpl(): Boolean {
        when (val preState = downloadState) {
            is DownloadState.Cancelable -> {
                ...
                // Always cancel the job, as it might be doing pre-processing or blocked by error checks
                preState.job.cancel()
            ...
            is DownloadState.Error -> {
                NotificationUtil.cancelNotification(notificationId)
                downloadState = DownloadState.Canceled(action = preState.action)
            }
            else -> {
                return false
            }
