...
import com.dartdl.app.App.Companion.startService
import com.dartdl.app.App.Companion.stopService
...
import com.dartdl.app.util.PlaylistEntry
...
object Downloader {
    ...
    fun onProcessCanceled(taskId: String) =
            mutableTaskList.run {
                get(taskId)?.let {
                    this.put(taskId, it.copy(state = CustomCommandTask.State.Canceled))
                }
            }
    ...
    fun manageDownloadError(
        ...
        context.makeToast(context.getString(resId))

        // Add a helpful hint for users to check troubleshooting
        applicationScope.launch(Dispatchers.Main) {
            kotlinx.coroutines.delay(2000) // Small delay so the first toast can be read
            context.makeToast(context.getString(R.string.download_error_hint))
        }
    ...
    fun cancelDownload() {
        context.makeToast(context.getString(R.string.task_canceled))
