...
fun matchUrlFromClipboard(string: String, isMatchingMultiLink: Boolean = false): String {
    findURLsFromString(string, !isMatchingMultiLink).joinToString(separator = "\n").run {
        if (isEmpty()) context.makeToast(R.string.paste_fail_msg)
        else context.makeToast(R.string.paste_msg)
        return this
    }
}
...
fun matchUrlFromSharedText(s: String): String {
    findURLsFromString(s, true).joinToString(separator = "\n").run {
        if (isEmpty()) context.makeToast(R.string.share_fail_msg)
        ...
        if (!firstMatchOnly) {
            while (find()) {
                ...
                if (PreferenceUtil.isPlayStoreBuild() && isYouTubeUrl(url)) {
                    context.makeToast("YouTube downloads are not supported due to Google Policy.")
                } else {
                    result += url
                }
            }
...
fun connectWithDelimiter(vararg strings: String?, delimiter: String): String =
        strings.toList().filter { !it.isNullOrBlank() }.joinToString(separator = delimiter) {
            it.toString()
        }
