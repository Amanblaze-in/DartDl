...
import com.dartdl.app.util.FileUtil.getFileProvider
import com.dartdl.app.util.PreferenceUtil.getInt
import com.dartdl.app.util.PreferenceUtil.updateLong
...
object UpdateUtil {
    ...
    private const val OWNER = "Amanblaze-in"
    private const val REPO = "DartDL"
    ...
    suspend fun updateYtDlp(): YoutubeDL.UpdateStatus? =
            withContext(Dispatchers.IO) {
                // Ensure engines are initialized before attempting update
                val isInit = App.initializeEngines().await()
                if (!isInit) {
                    // One more try with force
                    if (!App.initializeEngines(force = true).await()) {
                        throw IllegalStateException("Failed to initialize engines for update. Please check permissions.")
                    }
                }
                val channel =
    ...
    suspend fun resetYtDlp(): YoutubeDL.UpdateStatus? =
            withContext(Dispatchers.IO) {
                if (App.initializeEngines().await() != true) {
                    // Try force init if it failed
                    if (App.initializeEngines(force = true).await() != true) {
                        throw IllegalStateException("Failed to initialize engines for reset. Please check permissions.")
                    }
                }
                PreferenceUtil.encodeString(YT_DLP_VERSION, "")
                kotlin.runCatching {
                    val noBackupDir = context.getDir("no_backup", Context.MODE_PRIVATE)
    ...
    private fun getLatestRelease(): Release =
            client.newCall(requestForReleases).execute().body.use {
                ...
                val latestRelease =
                        releaseList
                                .filter {
                                    if (stable) it.name.toVersion() is Version.Stable else true
                                }
    ...
    fun checkForUpdate(context: Context = App.context): Release? {
        if (PreferenceUtil.isPlayStoreBuild()) return null
    ...
    fun installLatestApk(context: Context = App.context) =
            context.run {
                if (PreferenceUtil.isPlayStoreBuild()) return@run
                kotlin
                        .runCatching {
                            val contentUri =
    ...
    suspend fun downloadApk(
            ...
            withContext(Dispatchers.IO) {
                if (PreferenceUtil.isPlayStoreBuild()) return@withContext emptyFlow()
                val apkVersion =
                        context.packageManager
