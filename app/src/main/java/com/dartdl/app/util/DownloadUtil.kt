...
import com.dartdl.app.App.Companion.audioDownloadDir
import com.dartdl.app.App.Companion.context
import com.dartdl.app.App.Companion.videoDownloadDir
import com.dartdl.app.BuildConfig
import com.dartdl.app.Downloader
import com.dartdl.app.Downloader.onProcessEnded
import com.dartdl.app.Downloader.onProcessStarted
import com.dartdl.app.Downloader.onTaskEnded
import com.dartdl.app.Downloader.onTaskError
import com.dartdl.app.Downloader.onTaskStarted
import com.dartdl.app.Downloader.toNotificationId
...
import com.dartdl.app.ui.page.settings.network.Cookie
import com.dartdl.app.util.FileUtil.getArchiveFile
import com.dartdl.app.util.FileUtil.getConfigFile
import com.dartdl.app.util.FileUtil.getCookiesFile
import com.dartdl.app.util.FileUtil.getExternalTempDir
import com.dartdl.app.util.FileUtil.getFileName
import com.dartdl.app.util.FileUtil.getSdcardTempDir
import com.dartdl.app.util.FileUtil.moveFilesToDestination
import com.dartdl.app.util.PreferenceUtil.COOKIE_HEADER
...
object DownloadUtil {
    ...
    fun getPlaylistOrVideoInfo(
            ...
            YoutubeDL.runCatching {
                if (BuildConfig.FLAVOR == "playStore" && isYouTubeUrl(playlistURL)) {
                ...
                context.makeToast(context.getString(R.string.fetching_playlist_info))
    ...
    private fun YoutubeDLRequest.enableAria2c(): YoutubeDLRequest =
            this.addOption("--downloader", "libaria2c.so")
                    .addOption("--downloader-args", "aria2c:-c -j 10 -x 10 -s 10 -k 1M")
            ...
            this.apply {
                downloadPreferences.run {
                    ...
                    if (downloadSubtitle) {
                        ...
                        subtitleLanguage.takeIf { it.isNotEmpty() }?.let {
    ...
    private fun insertSplitChapterIntoHistory(videoInfo: VideoInfo, filePaths: List<String>) =
            filePaths.onEach {
                DatabaseUtil.insertInfo(
                        videoInfo
                                .toDownloadedVideoInfo(videoPath = it)
        ...
        with(downloadPreferences) {
            ...
            if (useDownloadArchive) {
                ...
                if (archiveFileContent.contains("${videoInfo.extractor} ${videoInfo.id}")) {
                    return Result.failure(
                            YoutubeDLException(context.getString(R.string.download_archive_error))
                    )
                }
            }
            ...
            request.apply {
                ...
                val destDir =
                        if (extractAudio || (videoInfo.vcodec == "none")) {
                            ...
                            if (privateDirectory) App.privateDownloadDir else audioDownloadDir
                        } else {
                            addOptionsForVideoDownloads(downloadPreferences)
                            if (privateDirectory) App.privateDownloadDir else videoDownloadDir
                        }

                val workingDir =
                        if (PreferenceUtil.isPlayStoreBuild() || destDir.startsWith("content://")) {
                ...
                if (BuildConfig.FLAVOR == "playStore" || (Build.VERSION.SDK_INT >= 29) || sdcard) {
                    addOption("-P", "temp:" + context.getExternalTempDir().absolutePath)
                } else if (Build.VERSION.SDK_INT > 23 && !sdcard) {
                    addOption("-P", "temp:" + context.getExternalTempDir().absolutePath)
                }
            ...
            return onFinishDownloading(
                    ...
                    destinationUri = if (sdcard) sdcardUri else pathBuilder.toString(),
            )
        }
    }
    ...
    private fun onFinishDownloading(
            ...
            videoInfo: VideoInfo,
            downloadPath: String,
            ytdlpOutputDir: String = downloadPath,
            ...
            preferences.run {
                ...
                if (PreferenceUtil.isPlayStoreBuild() || sdcard || destinationUri.startsWith("content://")) {
                    FileUtil.moveFilesToDestination(
                                    destinationUri = destinationUri,
                                    tempPath = if (sdcard) context.getSdcardTempDir(videoInfo.id) else File(downloadPath),
                            )
                            .onSuccess {
                                if (privateMode) {
    ...
    suspend fun executeCommandInBackground(
        ...
        downloadPreferences.run {
            ...
            context.makeToast(context.getString(R.string.start_execute))
            val request =
                    YoutubeDLRequest(urlList).apply {
                        commandDirectory.takeIf { it.isNotEmpty() }?.let { addOption("-P", it) }
                        addOption("--newline")
