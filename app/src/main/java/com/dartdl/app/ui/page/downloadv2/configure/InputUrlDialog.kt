...
import com.dartdl.app.ui.component.OutlinedDismissButton
...
import com.dartdl.app.util.findURLsFromString
...
private fun URLSelectionDialog(
    ...
    DartDLDialog(
            ...
            title = {
                Text(stringResource(R.string.select_multiple_link, urlListFromClipboard.size))
            },
                ...
                Box(modifier = Modifier.fillMaxSize()) {
                    ...
                    LazyColumn(modifier = Modifier.padding(bottom = 48.dp).heightIn(max = 600.dp)) {
                        itemsIndexed(urlListFromClipboard) { index, url ->
                            DialogCheckBoxItemVariant(
                                    text = url,
                                    checked = indexList.contains(index)
                            ) {
                                if (!it) {
                                    indexList -= index
                                } else {
